...
export async function GET(
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      let lastSeenCount = await prisma.facilityComment
        .count({ where: { facilityId: id } })
        .catch(() => 0);
      ...
      const poll = async () => {
        try {
          const currentCount = await prisma.facilityComment.count({
            where: { facilityId: id },
          });

          if (currentCount > lastSeenCount) {
            const newComments = await prisma.facilityComment.findMany({
              where: { facilityId: id, createdAt: { gt: new Date(Date.now() - 60_000) } },
